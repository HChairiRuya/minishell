...
/*   Updated: 2023/07/15 16:42:09 by hchairi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	syntaxe_err(void)
{
	...
	{
		redirect_err(node);
		if ((node->quotes == 0 && node->type == S_ERR) || ft_pipe(node))
			return (1);
		node = node->next;
	}
	if (node && (node->type == PIPES || node->type == RED_IN
			|| node->type == RED_OUT || node->type == APPEND
			|| node->type == HERDOC || node->type == S_ERR))
		return (1);
	return (0);
}
